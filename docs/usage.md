...
## Weather Integration (Canada only)
...
```python
# Traffic noise indicators and roughness
advanced_profile = nm.profile.periodic(
    df_1s,
    hour1=7,
    hour2=19,
...
nm.display.line(
    advanced_profile,
    'TNI', 'NPL',
    title='Traffic Noise Index and NPL'
)
```
...
# Visualize as heatmap
nm.display.freq_map(freq_series['Leq'], title="Frequency Evolution")
```

![Frequency Bands Time Series](images/freq_series_heatmap.png)

```python
# Daily profiles per frequency band
freq_daily = nm.profile.freq_periodic(
    df_freq,
    hour1=0,
    hour2=23,
...
Most functions support data coverage validation to ensure sufficient data quality.

```python
# Require 80% data coverage
with_coverage_leq = nm.summary.leq(
    df_1m,
    hour1=12,
    hour2=16,
...
A warning is raised if data coverage is not met for any computed period.
